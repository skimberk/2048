...
public class TwosLogic {
	public static int mergeLeft(int[] numbers, int from, int to) {
		int i = from;
		int score = 0;
...
//			System.out.println();
	...
	public static int mergeLeftRows(int[] numbers) {
		int size = (int) Math.sqrt(numbers.length);
		int score = 0;
		
		for(int i = 0; i < size; i++) {
			score += mergeLeft(numbers, i * size, i * size + size);
		}
		
		return score;
	}
	
	public static int mergeRightRows(int[] numbers) {
		reverseRows(numbers);
		int score = mergeLeftRows(numbers);
		reverseRows(numbers);
		
		return score;
	}
	
	public static int mergeUpRows(int[] numbers) {
		transpose(numbers);
		int score = mergeLeftRows(numbers);
		transpose(numbers);
	...
	public static int mergeDownRows(int[] numbers) {
		transpose(numbers);
		int score = mergeRightRows(numbers);
		transpose(numbers);
